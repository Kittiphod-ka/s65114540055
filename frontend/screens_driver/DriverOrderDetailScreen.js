...
export default function DriverOrderDetailScreen({ route, navigation }) {
  ...
  const fetchBookingStatus = async () => {
    try {
      const response = await axios.get(`http://10.0.2.2:5000/api/bookings/get-status2/${bookingData._id}`);
  ...
  const updateDriverLocation = async () => {
    try {
      ...
      await axios.post("http://10.0.2.2:5000/api/bookings/update-driver-location", {
  ...
  const handleAcceptOrder = async () => {
    ...
    try {
        const response = await axios.post("http://10.0.2.2:5000/api/bookings/update-status", {
            _id: bookingData._id,
...
const handleCancelOrder = async () => {
  ...
  try {
      const response = await axios.post("http://10.0.2.2:5000/api/bookings/cancel-order", {
          _id: bookingData._id,
          status: "ยกเลิก",
          driver_id: driverId,
      });
  ...
  const checkArrival = async (driverPos) => {
    ...
    if (distance < 100) {  
      if (status2 === "กำลังไปรับ" && !notifiedPickup.current) {
        ...
        await axios.post(`http://10.0.2.2:5000/api/bookings/update-status2`, {
          _id: bookingData._id,
          status2: "กำลังไปส่ง",
        });
  ...
  const handleCompleteOrder = async () => {
    await axios.post(`http://10.0.2.2:5000/api/bookings/update-status/:id`, {
      _id: bookingData._id,
      status: "เสร็จสิ้น",
      driver_id: driverId,
    });
