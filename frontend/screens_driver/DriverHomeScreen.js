...
export default function DriverHomeScreen({ navigation }) {
  ...
  const fetchDriverInfo = async () => {
    try {
      ...
      if (storedDriverId) {
        ...
        const response = await axios.get(`http://10.0.2.2:30055/api/drivers/status/${storedDriverId}`);
  ...
  const toggleAvailability = async () => {
    try {
      ...
      const response = await axios.post("http://10.0.2.2:30055/api/drivers/update-status", {  
