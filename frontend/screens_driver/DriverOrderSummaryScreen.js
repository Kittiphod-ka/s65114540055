...
export default function DriverOrderSummaryScreen({ route }) {
  ...
  const fetchImages = async () => {
    try {
      const response = await axios.get(`http://10.0.2.2:40055/api/booking-images/${bookingData._id}`);
  ...
  const handleUploadImage = async () => {
    ...
    if (!result.canceled) {
      ...
      try {
        const response = await axios.post("http://10.0.2.2:40055/api/booking-images/upload", formData, {
      ...
      <ScrollView horizontal>
        {images.length > 0 ? (
          images.map((img, index) => (
            <Image
              key={index}
              source={{ uri: `http://10.0.2.2:40055/uploads/${img.imageUrl.replace("/uploads/", "")}` }}
              style={styles.image}
            />
          ))
        ) : (
