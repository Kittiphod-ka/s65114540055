      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageContainer}>
        {images.length > 0 ? (
          images.map((img, index) => (
            <TouchableOpacity key={index} onPress={() => setSelectedImage(`http://10.0.2.2:40055${img.imageUrl}`)}>
              <Image source={{ uri: `http://10.0.2.2:40055${img.imageUrl}` }} style={styles.image} />
            </TouchableOpacity>
          ))
        ) : (
