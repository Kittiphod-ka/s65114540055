...
const BookingDetail = () => {
  ...
  const fetchBooking = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:30055/api/bookings/${id}`, {
  ...
  const fetchImages = async (bookingId) => {
    try {
      const response = await axios.get(`http://localhost:30055/api/booking-images/${bookingId}`);
            ...
            <div className="flex flex-wrap mt-2">
              {images.length > 0 ? (
                images.map((img, index) => (
                  <img
                    key={index}
                    src={`http://localhost:30055${img.imageUrl}`}
                    alt={`booking-${index}`}
