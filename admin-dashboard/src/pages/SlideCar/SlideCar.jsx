...
const SlideCar = () => {
  ...
  const fetchSlideCars = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/slidecars`); // ✅ ตรวจสอบเส้นทาง API
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      ...
      const response = await axios.put(`${import.meta.env.VITE_API_URL}/api/slidecars/update-status/${id}`, { status: newStatus });
